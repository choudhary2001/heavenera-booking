...
export default function RoomOwners() {
    ...
    // Fetch room owners from the API
    const fetchRoomOwners = useCallback(async () => {
        try {
            const response = await apiClient.get(`${baseURL}/api/admin/room-owners/`, {
                params: {
                    page: currentPage,
                    page_size: pageSize,
                    search: searchQuery,
            ...
            console.error("Error fetching room owners:", error);
        }
    ...
    // Handle deleting a room owner
    const handleDelete = async (id) => {
        try {
            await apiClient.delete(`${baseURL}/api/admin/room-owners/${id}/`);
            fetchRoomOwners();
        } catch (error) {
    ...
    // Open the modal for adding a new room owner
    const openModal = () => {
        setFormData({
            owner: "",
            phone: "",
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editMode) {
                await apiClient.put(`${baseURL}/api/admin/room-owners/${editId}/`, formDataToSend, {
                    headers: {
                        "Content-Type": "multipart/form-data",
                    },
                });
            } else {
                await apiClient.post(`${baseURL}/api/admin/room-owners/`, formDataToSend, {
                    headers: {
                        "Content-Type": "multipart/form-data",
                    },
                });
    ...
    return (
        <div className="flex flex-wrap mt-4">
            <div className="w-full mb-12 px-4">
                ...
                {/* Modal for adding/editing room owners */}
                <ReusableModal
                    title={editMode ? "Edit Room Owner" : "Add New Room Owner"}
                    isOpen={isModalOpen}
                    onClose={closeModal}
                    onSubmit={handleSubmit}
                    submitText={editMode ? "Update Room Owner" : "Add Room Owner"}
